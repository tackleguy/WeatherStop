// Subset of CONUS NEXRAD WSR-88D sites used by the source resolver to
// pick the nearest radar. The full 158-site inventory loads at runtime
// from https://api.weather.gov/radar/stations into the StationInventory
// modal; this list is the offline fallback so layer resolution works
// before that fetch completes.
...
export function nearestSite(lon: number, lat: number): NexradSite {
  // Haversine on the unit sphere; accurate enough for picking the nearest
  // station inside CONUS (no antimeridian crossings to worry about).
  const toRad = (d: number) => (d * Math.PI) / 180;
  let bestSite = NEXRAD_SITES[0];
  let bestDist = Infinity;
  const phi1 = toRad(lat);
  for (const s of NEXRAD_SITES) {
    const phi2 = toRad(s.lat);
    const dPhi = toRad(s.lat - lat);
    const dLam = toRad(s.lon - lon);
    const a =
      Math.sin(dPhi / 2) ** 2 +
